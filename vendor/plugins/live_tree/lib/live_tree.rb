...
module LiveTree
	...
	module ClassMethods
		...
		#
		# Either one of <tt>:model</tt> or <tt>:model_class_name</tt> must be specified.
		#
		# If <tt>:get_item_name_proc</tt> is not specified, the item's <tt>name</tt> attribute is used.
		#
		# If <tt>:get_item_children_proc</tt> is not specified, the item's <tt>children</tt> attribute is used.
		#
		# If <tt>:get_item_parent_proc</tt> is not specified, the item's <tt>parent</tt> attribute is used.
		#
	...
	module LiveTreeHelper
		...
		def _recurse_live_tree_data(item, depth, get_item_name_proc, get_item_children_proc, get_item_parent_proc, special_child_id = nil, special_child_data = nil) #:nodoc:
			result = "{id:" + item.id.to_s + ",name:'" + escape_javascript(get_item_name_proc.call(item).to_s) + "'"
      children = get_item_children_proc.call(item)
			if children.size == 0
				result += ",children:[]"
				...
				for child in children
					...
					if child.id == special_child_id
						...
						result += _recurse_live_tree_data(child, depth == nil ? nil : depth - 1, get_item_name_proc, get_item_children_proc, get_item_parent_proc, special_child_id, special_child_data)
					end
				end
				result += "]"
			end
		...
		def _get_live_tree_data(item, options, params) #:nodoc:
			...
			result = _recurse_live_tree_data(item, depth, get_item_name_proc, get_item_children_proc, get_item_parent_proc)
			if include_parents
				while get_item_parent_proc.call(item) != nil && (root_item_id == nil || item.id != root_item_id)
					result = _recurse_live_tree_data(get_item_parent_proc.call(item), 2, get_item_name_proc, get_item_children_proc, get_item_parent_proc, item.id, result)
